...
import { MenuFoldOutlined, MenuUnfoldOutlined } from "@ant-design/icons";import AppImage from "../../components/AppImage";
const shayariCards = [
  {
    ...
    <>      <Main>
        <div className="bg-black min-h-screen text-white">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-start p-6 md:p-12 bg-gradient-to-r from-indigo-700 via-purple-800 to-black">
            {/* Left: Image */}
            <div className="relative h-40 w-40 overflow-hidden rounded-lg shadow-lg md:h-60 md:w-60">
              <AppImage
                src="https://rekhta.pc.cdn.bitgravity.com/Images/Cms/T20/couplets-of-amjad-islam-amjad_Medium.png"
                alt="Banner Image"
                fill
              ...
              {shayariCards.map((card, index) => (
                <div key={index} className="px-2">
                  <div className="bg-zinc-800 rounded-2xl p-4 shadow-md hover:bg-zinc-700 transition-all duration-300 h-full">
                    <AppImage src={card.image} alt={card.title} width={600} height={160} className="mb-4 h-40 w-full rounded-xl object-cover" />
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0d2b80] via-black to-black text-white px-6 py-12 flex flex-col items-center animate-slide-up">
      ...
      <div className="backdrop-blur-md bg-white/10 border border-white/20 rounded-3xl shadow-2xl max-w-5xl w-full p-8 flex flex-col md:flex-row gap-8">
        {/* Left - Poster */}
        <AppImage src={data.image} alt={data.name} width={288} height={288} className="h-72 w-72 rounded-2xl object-cover shadow-lg transition-transform duration-500 hover:scale-105" />
