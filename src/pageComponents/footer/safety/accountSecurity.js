...
const getSecurityIcon = (title = "") => {
  ...
  if (normalizedTitle.includes("password") || normalizedTitle.includes("key")) return <KeyOutlined className="text-3xl text-[#F1C56A]" />;
  if (normalizedTitle.includes("login") || normalizedTitle.includes("device")) return <LaptopOutlined className="text-3xl text-[#F1C56A]" />;
  if (normalizedTitle.includes("phishing") || normalizedTitle.includes("report") || normalizedTitle.includes("help")) return <QuestionCircleOutlined className="text-3xl text-[#F1C56A]" />;

  return <SafetyCertificateOutlined className="text-3xl text-[#F1C56A]" />;
};
            ...
            <div className="mt-10 grid w-full gap-8">
              {loading
                ...
                : securityFeatures.map((feature, index) => (
                    ...
                    >
                      ...
                      <div>
                        <h3 className="text-xl font-semibold text-[#F6E7C8]">{feature.title}</h3>
                        <p className="mt-2 mb-5 text-sm leading-7 text-[#D0D0D0] sm:text-[15px]">{feature.description}</p>
                        <Link to={feature.link} className="inline-flex items-center gap-1 font-semibold text-[#F1C56A] transition-colors duration-300 hover:text-[#FFD97A]">
                          {feature.linkText}
              ...
              {!loading && securityFeatures.length === 0 && (
                <div className="rounded-[24px] border border-[rgba(212,175,55,0.16)] bg-[rgba(255,255,255,0.03)] px-6 py-10 text-center text-[#CFC3B0] shadow-[0_18px_60px_rgba(0,0,0,0.32)] backdrop-blur-xl">
                  No account security data available right now.
                </div>
              )}
            </div>
          </div>
